...
var JwksClient = exports.JwksClient = function () {
  function JwksClient(options) {
    ...
    this.getSigningKey = function (token, cb) {
      var kid = token.header.kid;

      _this.logger('Fetching signing key for \'' + kid + '\'');

      _this.getSigningKeys(token, function (err, keys) {
        if (err) {
          return cb(err);
        }
  ...
  _createClass(JwksClient, [{
    key: 'getKeys',
    value: function getKeys(token, cb) {
      var _this2 = this;

      var uri = this.options.jwksUri instanceof Function ? this.options.jwksUri(token) : this.options.jwksUri;
    ...
    key: 'getSigningKeys',
    value: function getSigningKeys(token, cb) {
      var _this3 = this;

      this.getKeys(token, function (err, keys) {
        if (err) {
          return cb(err);
        }
